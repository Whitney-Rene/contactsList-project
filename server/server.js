...
app.delete("/delete-contact/:contactId", async (req, res) => {
  ...
  try {
    ...
    const deleteQuery = "DELETE FROM contacts WHERE contact_id = $1";

    // Ensure you have a valid connection before executing the query
    const result = await db.query(deleteQuery, [contactId]);
